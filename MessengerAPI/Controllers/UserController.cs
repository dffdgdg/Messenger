    ...
    : BaseController<UserController>(logger)
{
    ...
    public async Task<ActionResult<ApiResponse<List<UserDTO>>>> GetAllUsers(
        ...
        => await ExecuteAsync(
            () => userService.GetAllUsersAsync(ct),
    ...
    public async Task<IActionResult> UpdateUser(
        int id, [FromBody] UserDTO userDto, CancellationToken ct)
    {
        if (!IsCurrentUser(id)) return Forbidden();
        return await ExecuteAsync(
            () => userService.UpdateUserAsync(id, userDto, ct),
        ...
        int id, IFormFile file, CancellationToken ct)
    {
        ...
        return await ExecuteAsync(
            () => userService.UploadAvatarAsync(id, file, ct),
            "Аватар загружен успешно");
    }

    [HttpPut("{id}/username")]
    public async Task<IActionResult> ChangeUsername(
        int id, [FromBody] ChangeUsernameDTO dto, CancellationToken ct)
    {
        if (!IsCurrentUser(id)) return Forbidden();
        return await ExecuteAsync(
            () => userService.ChangeUsernameAsync(id, dto, ct),
            "Username успешно изменён");
    }

    [HttpPut("{id}/password")]
    public async Task<IActionResult> ChangePassword(
        int id, [FromBody] ChangePasswordDTO dto, CancellationToken ct)
    {
        if (!IsCurrentUser(id)) return Forbidden();
        return await ExecuteAsync(
            () => userService.ChangePasswordAsync(id, dto, ct),
    ...
    public async Task<ActionResult<ApiResponse<OnlineUsersResponseDTO>>> GetOnlineUsers(
        CancellationToken ct)
        => await ExecuteAsync(
            () => userService.GetOnlineUsersAsync(ct),
            "Список онлайн пользователей получен");

    [HttpGet("{id}/status")]
    public async Task<ActionResult<ApiResponse<OnlineStatusDTO>>> GetUserOnlineStatus(
        int id, CancellationToken ct)
        => await ExecuteAsync(
    ...
    public async Task<ActionResult<ApiResponse<List<OnlineStatusDTO>>>> GetUsersOnlineStatus(
        [FromBody] List<int> userIds, CancellationToken ct)
        => await ExecuteAsync(
