...
public partial class ChatService(
    ...
    ILogger<ChatService> logger) : BaseService<ChatService>(unitOfWork, logger), IChatService
{
    ...
    public async Task<Result<List<ChatDto>>> GetUserChatsAsync(int userId)
    {
        ...
        var userRoles = await chatRepository.GetMemberRolesAsync(chatIds, userId);
    ...
    public async Task<Result<ChatDto>> GetChatForUserAsync(int chatId, int userId)
    {
        ...
        var member = await chatRepository.GetMemberAsync(chatId, userId);
        var role = member?.Role ?? default;
    ...
    public async Task<Result<ChatDto>> CreateChatAsync(ChatDto dto, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var memberIds = await chatRepository.GetMemberIdsAsync(newChat.Id, ct);

            foreach (var memberId in memberIds)
            {
                await _hubNotifier.SendToUserAsync(memberId, HubMethods.Chat.ChatUpdated, createdEvent);
                await _hubNotifier.AddUserToChatGroupAsync(memberId, newChat.Id);
            }
    ...
    private async Task<API.Domain.Entities.Chat> PersistNewChatAsync(ChatDto dto, int? contactUserId, CancellationToken ct)
    {
        var newChat = new API.Domain.Entities.Chat
        {
    ...
    private async Task<Result> ApplyChatUpdatesAsync(API.Domain.Entities.Chat chatEntity, UpdateChatDto dto, int userId, int chatId)
    {
    ...
    private ChatUpdateEventDto BuildUpdateEvent(API.Domain.Entities.Chat chatEntity) => new()
    {
        Id = chatEntity.Id,
        Name = chatEntity.Name,
        Type = chatEntity.Type,
    ...
    {
        public API.Domain.Entities.Chat Chat { get; init; } = null!;
