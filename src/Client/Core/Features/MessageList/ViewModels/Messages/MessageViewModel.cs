...
using Core.Features.MessageList.ViewModels.Messages;
...
public sealed partial class MessageViewModel : ObservableObject, IDisposable
{
    ...
    public MessageViewModel(MessageDto message, IFileDownloadService? downloadService = null, INotificationService? notificationService = null,
        ...
        IFileDownloadStateService? stateService = null, IDownloadManager? downloadManager = null)
    {
        (_downloadService, _notificationService, _stateService, _audioPlayer, _apiClient, _currentUserId, Message, _downloadManager) =
            (downloadService, notificationService, stateService, audioPlayer, apiClient, currentUserId, message, downloadManager);
    ...
    private void InitChildViewModels(MessageDto m)
    {
        ...
        if (Files.Count > 0)
        {
            FileViewModels = new(Files.Select(f => new MessageFileViewModel(f, _downloadManager, _downloadService, _notificationService, _stateService)));
    ...
    private async Task PersistPollStateToCacheAsync()
    {
        ...
        Message.Files = [];
        try { await AppConfig.Services.GetRequiredService<ILocalCacheService>().UpsertMessageAsync(Message); }
        catch { }
    ...
    {
        if (string.IsNullOrEmpty(VoiceFileUrl) || _downloadManager is null) return;

        try
        {
            var voiceFile = new MessageFileDto
            {
                Id = Id,
                MessageId = Id,
                FileName = $"voice_{Id}_{CreatedAt:yyyyMMdd_HHmmss}.wav",
                ContentType = "audio/wav",
                Url = BuildMediaUrl(VoiceFileUrl),
                FileSize = 0
            };

            var outcome = await _downloadManager.DownloadAsync(voiceFile);
            if (outcome.Status == DownloadOutcomeStatus.Completed && outcome.FilePath is not null)
                _notificationService?.ShowSuccessAsync($"Голосовое сохранено: {outcome.FilePath}", copyToClipboard: false);
            else if (outcome.Status == DownloadOutcomeStatus.Failed)
                _notificationService?.ShowErrorAsync($"Ошибка: {outcome.Error}", copyToClipboard: false);
        }
        catch (Exception ex)
        {
            _notificationService?.ShowErrorAsync($"Ошибка: {ex.Message}", copyToClipboard: false);
        }
    }
    ...
    {
        if (!Uri.TryCreate(url, UriKind.Absolute, out var absolute)) return $"{AppConfig.ApiUrl.TrimEnd('/')}/{url.TrimStart('/')}";
        if (!Uri.TryCreate(AppConfig.ApiUrl, UriKind.Absolute, out var api)) return absolute.ToString();
