...
public class ChatRepositoryTests : IntegrationTestBase
{
    ...
    public async Task FindByIdWithMembers_ReturnsChatWithMembers()
    {
        ...
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id, ChatType.Chat, "Group", [user.Id]);
    ...
    {
        var result = await _repo.FindByIdWithMembersAsync(999);
    ...
    public async Task FindContactChat_ReturnsChat_WhenExists()
    {
        ...
        var chat = await DbContextFactory.SeedContactChatAsync(Context, u1.Id, u2.Id);
    ...
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "u1", "pass");
        var u2 = await DbContextFactory.SeedUserAsync(Context, "u2", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Test", [u1.Id, u2.Id]);
    ...
    {
        var ids = await _repo.GetMemberIdsAsync(999);
        ids.Should().BeEmpty();
    }
    ...
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "member", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Test", [u1.Id]);
    ...
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "member", "pass");
        var u2 = await DbContextFactory.SeedUserAsync(Context, "outsider", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Test", [u1.Id]);
    ...
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "creator", "pass");
        var u2 = await DbContextFactory.SeedUserAsync(Context, "newbie", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Test", [u1.Id]);
    ...
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "owner", "pass");
        var u2 = await DbContextFactory.SeedUserAsync(Context, "leaver", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Test", [u1.Id, u2.Id]);
    ...
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "u1", "pass");
        var chat1 = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Chat1", [u1.Id]);
        var chat2 = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Chat2", [u1.Id]);
    ...
    {
        var result = await _repo.GetShowHistoryForNewMembersAsync(999);
        result.Should().BeNull();
    }
    ...
    public async Task GetLastMessages_ReturnsProjections()
    {
        ...
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Test", [u1.Id]);
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Hello world");
    ...
    public async Task GetMembersWithUsers_ReturnsMemberProjections()
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "alice", "pass");
        var u2 = await DbContextFactory.SeedUserAsync(Context, "bob", "pass");
        var chat = await DbContextFactory.SeedGroupChatAsync(Context, u1.Id, "Group", u2.Id);
    ...
    public async Task GetContactChatsWithMembers_ReturnsContactChats()
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "alice", "pass");
        var u2 = await DbContextFactory.SeedUserAsync(Context, "bob", "pass");
        var chat1 = await DbContextFactory.SeedContactChatAsync(Context, u1.Id, u2.Id);
        var chat2 = await DbContextFactory.SeedGroupChatAsync(Context, u1.Id, "Group", u2.Id);
    ...
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "admin", "pass");
        await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Developers", [u1.Id]);
        await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Designers", [u1.Id]);
        await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Marketing", [u1.Id]);
    ...
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "owner", "pass");
        var u2 = await DbContextFactory.SeedUserAsync(Context, "member", "pass");
