...
{
    public async Task<Dictionary<int, ChatRole>> GetMemberRolesAsync(IEnumerable<int> chatIds, int userId, CancellationToken ct = default)
    {
        var ids = chatIds.ToList();
        return await _context.ChatMembers.Where(cm => ids.Contains(cm.ChatId) && cm.UserId == userId).ToDictionaryAsync(cm => cm.ChatId, cm => cm.Role, ct);
    }

    public Task<Chat?> GetChatWithTrackingAsync(int chatId, CancellationToken ct = default)
        => _context.Chats.FirstOrDefaultAsync(c => c.Id == chatId, ct);
    ...
    public Task<List<int>> GetMemberIdsForUserAsync(int userId, CancellationToken ct = default)
        => _context.ChatMembers.Where(cm => cm.UserId == userId).Select(cm => cm.ChatId).ToListAsync(ct);
    ...
    public Task<List<Chat>> GetChatsByIdsAsync(IEnumerable<int> chatIds, CancellationToken ct = default)
        => _context.Chats.Where(c => chatIds.Contains(c.Id)).AsNoTracking().ToListAsync(ct);
