...
public sealed class TokenService : ITokenService
{
    ...
    public TokenService(IOptions<JwtSettings> settings)
    {
        ...
        {
            throw new InvalidOperationException("JWT Secret не настроен. Установите 'Jwt:Secret' через переменную окружения или user-secrets.");
        }
    ...
    public static TokenValidationParameters CreateValidationParameters(IConfiguration config)
    {
            ...
            || keyMaterial == SecretPlaceholder)
        {
            throw new InvalidOperationException(
                "Jwt:Secret не настроен. Установите через переменную окружения или user-secrets.");
        }
        ...
        return new TokenValidationParameters
        {
            ...
            ValidateIssuer = true,
            ValidIssuer = config["Jwt:Issuer"] ?? "API",
            ValidateAudience = true,
            ValidAudience = config["Jwt:Audience"] ?? "MessengerClient",
            ValidateLifetime = true,
            ClockSkew = TimeSpan.Zero
        };
    }
