...
public class ReadReceiptRepositoryTests : IntegrationTestBase
{
    ...
    public async Task CountUnread_ReturnsCorrectCount()
    {
        ...
        var msg1 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Msg1");
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Msg2");
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Msg3");
    ...
    public async Task GetUnreadCountsAsync_ReturnsDictionary()
    {
        ...
        var u2 = await DbContextFactory.SeedUserAsync(Context, "bob", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Test", [u1.Id, u2.Id]);
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Hello");
    ...
    public async Task MessageExists_True()
    {
        ...
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Exists");
    ...
    public async Task MessageExists_False_WhenDeleted()
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "alice", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Will delete");
        msg.IsDeleted = true;
    ...
    public async Task GetLastMessageId_ReturnsMaxId()
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "alice", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id);
        var msg1 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "First");
        var msg2 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Last");
    ...
    public async Task GetUnreadCountsForUsers_ReturnsPerUserCounts()
    {
        var u1 = await DbContextFactory.SeedUserAsync(Context, "alice", "pass");
        var u2 = await DbContextFactory.SeedUserAsync(Context, "bob", "pass");
        var u3 = await DbContextFactory.SeedUserAsync(Context, "charlie", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, u1.Id, ChatType.Chat, "Test", [u1.Id, u2.Id, u3.Id]);
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, u1.Id, "Hello all");
