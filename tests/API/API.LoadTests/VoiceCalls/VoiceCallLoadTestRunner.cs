...
public sealed class VoiceCallLoadTestRunner(VoiceCallLoadTestOptions options, ICallSessionService callSessions,
    ILogger<VoiceCallLoadTestRunner> logger)
{
    ...
    private sealed class VirtualVoiceParticipant : IDisposable
    {
        ...
        private static byte[] CreateVoiceFrame(int userId)
        {
            ...
            if (bytes <= 0)
                throw new InvalidOperationException("Не удалось подготовить Opus-фрейм для нагрузочного теста.");
