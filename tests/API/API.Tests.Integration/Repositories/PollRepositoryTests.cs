...
public class PollRepositoryTests : IntegrationTestBase
{
    ...
    {
        var (poll, _) = await TestDataSeeder.SeedPollAsync(Context);
    ...
    {
        var (poll, msg) = await TestDataSeeder.SeedPollAsync(Context);
    ...
    {
        var (poll, _) = await TestDataSeeder.SeedPollAsync(Context);
        var user = await TestDataSeeder.SeedUserAsync(Context, "voter", "pass");
