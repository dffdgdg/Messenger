...
public partial class MainMenuViewModel : BaseViewModel
{
        ...
        switch (index)
        {
            ...
            case 6:
                if (_departmentViewModel == null)
                {
                    ...
                    {
                        var dialog = new ConfirmDialogViewModel("Óäàëåíèå èç îòäåëà",
                            $"Âû óâåðåíû, ÷òî õîòèòå óäàëèòü {member.DisplayName} èç îòäåëà?",
                            "Óäàëèòü","Îòìåíà");
                    ...
                    {
                        var dialog = new SelectUserDialogViewModel(users, "Äîáàâèòü ñîòðóäíèêà");
                        await _mainWindowViewModel.ShowDialogAsync(dialog);
                        return await dialog.Result;
                    };
                }
    ...
    /// <summary>
    /// Ïåðåêëþ÷èòüñÿ íà íóæíóþ âêëàäêó è îòêðûòü ÷àò (óíèâåðñàëüíûé ìåòîä)
    /// </summary>
    public async Task SwitchToTabAndOpenChatAsync(ChatDto chat)
    {
        // Îïðåäåëÿåì íóæíóþ âêëàäêó ïî òèïó ÷àòà
        bool isGroupChat = chat.Type == ChatType.Chat || chat.Type == ChatType.Department;

        if (isGroupChat)
        {
            // Èñïîëüçóåì ñóùåñòâóþùèé ìåòîä äëÿ ãðóïï
            await OpenChatAsync(chat);
        }
        else
        {
            // Äëÿ êîíòàêòîâ ïåðåêëþ÷àåìñÿ íà âêëàäêó êîíòàêòîâ
            SetActiveMenu(5);
            await Task.Delay(50);
    ...
    /// <summary>
    /// Ïåðåêëþ÷èòüñÿ íà íóæíóþ âêëàäêó è îòêðûòü ñîîáùåíèå
    /// </summary>
    ...
    public async Task ShowUserProfileAsync(int userId) => await SafeExecuteAsync(async () =>
    {
        ...
        {
            ErrorMessage = $"Íå óäàëîñü çàãðóçèòü ïðîôèëü: {result.Error}";
        }
    });
    ...
    public async Task ShowPollDialogAsync(int chatId, Action? onPollCreated = null)
    {
        ...
        {
            ErrorMessage = $"Îøèáêà îòêðûòèÿ äèàëîãà: {ex.Message}";
        }
    }
    ...
    private async Task CreatePollAsync(CreatePollDto dto) => await SafeExecuteAsync(async () =>
    {
        ...
        if (result.Success)
            SuccessMessage = "Îïðîñ ñîçäàí";
        else
            ErrorMessage = $"Îøèáêà ñîçäàíèÿ îïðîñà: {result.Error}";
    });
    ...
    /// <summary>
            ...
            var membersResult = await _apiClient.GetAsync<List<ChatMemberDto>>(ApiEndpoints.Chat.MembersDetailed(chat.Id));
                SaveAction = async (chatDto, memberIds, adminIds, avatarStream, avatarFileName)
                => await UpdateGroupChatAsync(chatDto, memberIds, adminIds, avatarStream, avatarFileName, onGroupUpdated),
                ShowDialogAction = dialogVm => _mainWindowViewModel.ShowDialogAsync(dialogVm)
        {
    private async Task<bool> CreateGroupChatAsync(ChatDto chatDto,List<int> memberIds,List<int> adminIds,Stream? avatarStream,string? avatarFileName,Action<ChatDto>? onSuccess)
            {
            foreach (var adminId in adminIds)
            {
                await _apiClient.PutAsync(ApiEndpoints.Chat.MemberRole(createdChat.Id, adminId, ChatRole.Admin), null!);
            }

    private async Task<bool> UpdateGroupChatAsync(ChatDto chatDto, List<int> memberIds, List<int> adminIds, Stream? avatarStream, string? avatarFileName, Action<ChatDto>? onSuccess)
            ...
            var currentAdminIds = currentMembers.Where(x => x.Role is ChatRole.Admin or ChatRole.Owner).Select(x => x.UserId).ToHashSet();

            foreach (var adminId in adminIds.Where(id => currentMemberIds.Contains(id) && !currentAdminIds.Contains(id)))
            {
                await _apiClient.PutAsync(ApiEndpoints.Chat.MemberRole(chatDto.Id, adminId, ChatRole.Admin), null!);
            }
            ...
            {
                await _apiClient.PutAsync(ApiEndpoints.Chat.MemberRole(chatDto.Id, memberId, ChatRole.Member), null!);
            }
    ...
    /// <summary>
    /// Ïîêàçàòü äèàëîã ðåäàêòèðîâàíèÿ ãðóïïû
    /// </summary>
    ...
    private async Task<bool> CreateGroupChatAsync(ChatDto chatDto,List<int> memberIds,Stream? avatarStream,string? avatarFileName,Action<ChatDto>? onSuccess)
    {
        try
        {
            ...
            {
                ErrorMessage = $"Îøèáêà ñîçäàíèÿ ãðóïïû: {createResult.Error}";
                return false;
            }
            ...
            SuccessMessage = "Ãðóïïà óñïåøíî ñîçäàíà";
    ...
    private async Task<bool> UpdateGroupChatAsync(ChatDto chatDto, List<int> memberIds, Stream? avatarStream, string? avatarFileName, Action<ChatDto>? onSuccess)
    {
        try
        {
            ...
            {
                ErrorMessage = $"Îøèáêà îáíîâëåíèÿ ãðóïïû: {updateResult.Error}";
                return false;
            }
            ...
            SuccessMessage = "Ãðóïïà óñïåøíî îáíîâëåíà";
