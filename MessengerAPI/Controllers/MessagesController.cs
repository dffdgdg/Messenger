    ...
    : BaseController<MessagesController>(logger)
{
        ...
        [FromBody] MessageDTO messageDto)
    {
        ...
        return await ExecuteAsync(
            () => messageService.CreateMessageAsync(messageDto),
            "Сообщение успешно отправлено");
    }

    [HttpPut("{id}")]
    public async Task<ActionResult<ApiResponse<MessageDTO>>> UpdateMessage(
        int id, [FromBody] UpdateMessageDTO updateDto)
        => await ExecuteAsync(async () =>
        {
            if (id != updateDto.Id)
                return Result<MessageDTO>.Failure("Несоответствие ID сообщения");
            return await messageService.UpdateMessageAsync(
                id, GetCurrentUserId(), updateDto);
        }, "Сообщение успешно отредактировано");

    [HttpDelete("{id}")]
    public async Task<IActionResult> DeleteMessage(int id)
        => await ExecuteAsync(
            () => messageService.DeleteMessageAsync(id, GetCurrentUserId()),
        ...
        [FromQuery] int pageSize = 15)
    {
        ...
        return await ExecuteAsync(
            () => messageService.GetChatMessagesAsync(
    ...
    public async Task<ActionResult<ApiResponse<PagedMessagesDTO>>> GetMessagesAround(
        int chatId, int messageId, [FromQuery] int count = 50)
    {
        var userId = GetCurrentUserId();
        return await ExecuteAsync(
            () => messageService.GetMessagesAroundAsync(
                chatId, messageId, userId, count));
    }

    [HttpGet("chat/{chatId}/before/{messageId}")]
    public async Task<ActionResult<ApiResponse<PagedMessagesDTO>>> GetMessagesBefore(
        int chatId, int messageId, [FromQuery] int count = 30)
    {
        var userId = GetCurrentUserId();
        return await ExecuteAsync(
            () => messageService.GetMessagesBeforeAsync(
                chatId, messageId, userId, count));
    }
        ...
        int chatId, int messageId, [FromQuery] int count = 30)
    {
        var userId = GetCurrentUserId();
        return await ExecuteAsync(
            () => messageService.GetMessagesAfterAsync(
                chatId, messageId, userId, count));
    }
        ...
        [FromQuery] int pageSize = 20)
    {
        ...
        return await ExecuteAsync(
            () => messageService.SearchMessagesAsync(
                chatId, userId, query, page, pageSize));
    }
        ...
        [FromQuery] int pageSize = 20)
    {
        ...
        return await ExecuteAsync(
            () => messageService.GlobalSearchAsync(userId, query, page, pageSize));
    }

    [HttpGet("{id}/transcription")]
    public async Task<ActionResult<ApiResponse<VoiceTranscriptionDTO>>> GetTranscription(
        int id, CancellationToken ct)
        => await ExecuteAsync(
            () => transcriptionService.GetTranscriptionAsync(id, ct));

    [HttpPost("{id}/transcription/retry")]
    public async Task<IActionResult> RetryTranscription(
        int id, CancellationToken ct)
        => await ExecuteAsync(
            () => transcriptionService.RetryTranscriptionAsync(id, ct),
