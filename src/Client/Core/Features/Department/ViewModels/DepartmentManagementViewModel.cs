...
namespace Core.Features.Department.ViewModels;
...
public partial class DepartmentManagementViewModel : BaseViewModel
{
    ...
    {
        if (IsBusy && Members.Count > 0) return;
        ...
        try
        {
            ...
            var userResult = await _apiClient.GetAsync<UserDto>(
                ApiEndpoints.Users.ById(CurrentUserId), ct);
            ...
            var canManageResult = await _apiClient.GetAsync<bool>(
                ApiEndpoints.Departments.CanManage((int)_departmentId), ct);
            ...
            var departmentResult = await _apiClient.GetAsync<DepartmentDto>(
                ApiEndpoints.Departments.ById((int)_departmentId), ct);
    ...
    private async Task AddMember()
    {
        ...
        await SafeExecuteAsync(async ct =>
        {
            ...
            var result = await _apiClient.PostAsync(ApiEndpoints.Departments.Members((int)_departmentId), dto, ct);
    ...
    private async Task RemoveMember(DepartmentMemberViewModel? member)
    {
        ...
        await SafeExecuteAsync(async ct =>
        {
            var result = await _apiClient.DeleteAsync(ApiEndpoints.Departments.RemoveMember((int)_departmentId, member.UserId), ct);
