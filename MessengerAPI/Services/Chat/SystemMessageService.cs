    ...
    : BaseService<SystemMessageService>(context, logger), ISystemMessageService
{
    public async Task CreateCallEndedMessageAsync(int chatId, int initiatorId, TimeSpan duration)
    {
        ...
        var durationText = duration.TotalHours >= 1 ? $"{(int)duration.TotalHours}:{duration.Minutes:D2}:{duration.Seconds:D2}" : $"{duration.Minutes}:{duration.Seconds:D2}";

        var message = new Message
        {
            ChatId = chatId,
            SenderId = initiatorId,
            Content = $"Звонок завершён · {durationText}",
            IsSystemMessage = true,
            SystemEventType = SystemEventType.CallEnded,
        ...
        var loaded = await _context.Messages.Include(m => m.Sender).AsNoTracking().FirstAsync(m => m.Id == message.Id);
    ...
    public async Task CreateCallStartedMessageAsync(int chatId, int initiatorId)
    {
        ...
        var message = new Message
        {
            ...
            Content = null,
            IsSystemMessage = true,
            SystemEventType = SystemEventType.CallStarted,
            CreatedAt = appDateTime.UtcNow
        };
