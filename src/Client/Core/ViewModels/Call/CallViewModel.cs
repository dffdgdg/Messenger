...
public partial class CallViewModel : BaseViewModel, IActiveCall
{
    ...
    private readonly ICallAudioService _audioService;

    public CallViewModel(
        ICallService callService,
        ICallHubConnection hub,
        ActiveCallStore store,
        ICallAudioService audioService)
    {
    ...
    // Весь остальной код без изменений — Initialize, команды, хаб-события, таймер, Dispose
    // ... (оставь как было, просто без #if !ANDROID обёртки)
    ...
    public void Initialize(CallStateDto state, string chatName, bool isGroupCall, int myUserId)
    {
        ...
        _elapsedSecondsAtStart = state.ElapsedSeconds;
        _localStartedAt = DateTimeOffset.UtcNow;
        Participants.Clear();

        var me = state.Participants.FirstOrDefault(p => p.UserId == myUserId);
        if (me != null) Participants.Add(new CallParticipantViewModel(me));
    ...
    private void OnMuteChangedExternally(bool isMuted) => Dispatcher.UIThread.Post(() =>
    {
        ...
        if (me != null) me.IsMuted = isMuted;
    });

    private void OnParticipantSpeakingChanged(int userId, bool isSpeaking) =>
        Dispatcher.UIThread.Post(() =>
        {
            var vm = Participants.FirstOrDefault(p => p.UserId == userId);
            if (vm != null) vm.IsSpeaking = isSpeaking;
        });

    private void OnLocalSpeakingStateChanged(bool isSpeaking) => Dispatcher.UIThread.Post(() =>
    {
        var me = Participants.FirstOrDefault(p => p.UserId == _myUserId);
        if (me != null) me.IsSpeaking = isSpeaking;
    });
    ...
    {
        var localElapsed = (int)(DateTimeOffset.UtcNow - _localStartedAt).TotalSeconds;
        var totalSeconds = _elapsedSecondsAtStart + localElapsed;
