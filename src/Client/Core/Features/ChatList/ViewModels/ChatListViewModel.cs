...
namespace Core.Features.ChatList.ViewModels;

public partial class ChatListViewModel : BaseViewModel, IRefreshable
{
    ...
    private ChatSearchHandler? _subscribedSearch;
    ...
    public bool CombinedIsInfoPanelVisible => CurrentChatViewModel?.InfoPanel.IsInfoPanelOpen == true;

    public ChatListViewModel(MainMenuViewModel parent, bool isGroupMode, IApiClientService apiClient, IAuthManager authManager,
    ...
    private void InitializeSearchManager()
    {
        ...
        SearchManager = new GlobalSearchManager(_authManager.Session.UserId.Value, IsGroupMode, _apiClient,
            getUsersFunc: LoadUsersForFilterAsync, getChatsFunc: LoadChatsForFilterAsync);
    ...
    private void SyncSearchScopeWithChatViewMode()
    {
        ...
        if (CurrentChatViewModel?.Search.IsSearchMode == true && SelectedChat != null &&
    ...
    private void CloseSearch()
    {
        ...
        if (CurrentChatViewModel?.Search.IsSearchMode == true)
            CurrentChatViewModel.Search.IsSearchMode = false;
    }
    ...
    private async Task OpenSearchFilters()
    {
        ...
        var dialog = new SearchFiltersDialogViewModel(SearchManager,
    ...
    private void OnUnreadCountChanged(int chatId, int unreadCount) =>
        FindChat(chatId)?.Apply(c => c.UnreadCount = unreadCount);
    ...
    private void OnMessageReceivedGlobally(MessageDto message)
    {
        ...
        chat.LastMessageSenderName =
            ChatPreViewFormatter.FormatSenderName(message.SenderName, message.SenderId, currentUserId);
    ...
    partial void OnCurrentChatViewModelChanged(ChatViewModel? oldValue, ChatViewModel? newValue)
    {
        ...
        if (_subscribedChatVm?.InfoPanel != null)
            _subscribedChatVm.InfoPanel.PropertyChanged -= OnInfoPanelPropertyChanged;

        if (_subscribedSearch != null)
            _subscribedSearch.PropertyChanged -= OnSearchPropertyChanged;

        _subscribedChatVm = newValue;
        _subscribedSearch = newValue?.Search;
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        if (disposing)
        {
            ...
            _subscribedChatVm = null;
            _subscribedSearch = null;
