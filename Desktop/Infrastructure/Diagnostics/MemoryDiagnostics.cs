...
public static class MemoryDiagnostics
{
    ...
    private static int _imgLargeSkipped; // элементы > 85 KB (попали бы в LOH)
    ...
    public static void RegisterImageLoader(
        Func<(int count, long bytes)> getStats,
        Action clearCache)
    {
    ...
    public static void Dump(string label)
    {
        ...
        var lohDelta = (long)lohBytes - (long)_prevLohBytes;
    ...
    public static void DumpDetailed(string label, bool runLohExperiment = false)
    {
        ...
        sb.AppendLine($"  LOH:     {info.GenerationInfo[3].SizeAfterBytes / 1024}KB");  // KB чтобы видеть малые изменения
        ...
        // LOH эксперимент — только когда явно просят, т.к. сайд-эффект (чистит кэш)
        if (runLohExperiment)
