...
public partial class AvatarControl : UserControl
{
    ...
    public static readonly StyledProperty<IImage?> ImageBitmapProperty = AvaloniaProperty.Register<AvatarControl, IImage?>(nameof(ImageBitmap));
    ...
    protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e)
    {
        ...
        PlaceholderBackground ??= this.FindResource("AccentLight") as IBrush ?? new SolidColorBrush(Color.Parse("#5B5FC7"));

        PlaceholderForeground ??= this.FindResource("AccentForeground") as IBrush ?? Brushes.White;
    ...
    private static string? GetExtension(string path)
    {
        ...
        return path[dotIndex..].ToLowerInvariant();
    }
    ...
    private static readonly IBrush BrushOnline = new SolidColorBrush(Color.Parse("#43A047"));
    private static readonly IBrush BrushAway = new SolidColorBrush(Color.Parse("#FFA000"));
    private static readonly IBrush BrushBusy = new SolidColorBrush(Color.Parse("#E53935"));
    private static readonly IBrush BrushDnd = new SolidColorBrush(Color.Parse("#9C27B0"));
    ...
    private void UpdateStatusColor()
    {
        ...
        var (brush, tooltip) = StatusType switch
        {
            UserStatusType.Online => (BrushOnline, "В сети"),
            UserStatusType.Away => (BrushAway, "Отошёл"),
            UserStatusType.Busy => (BrushBusy, "Занят"),
            UserStatusType.DoNotDisturb => (BrushDnd, "Не беспокоить"),
            _ => (BrushOnline, "В сети")
        };

        OnlineIndicatorBackground = brush;
