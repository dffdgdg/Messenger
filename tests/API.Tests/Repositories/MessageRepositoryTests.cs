...
public class MessageRepositoryTests : IntegrationTestBase
{
    ...
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Hello");
    ...
    public async Task FindUserMessageWithIncludes_ReturnsWithRelations()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Test");
    ...
    public async Task GetBefore_ReturnsOlderMessages()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg1 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "First");
        var msg2 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Second");
        var msg3 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Third");
    ...
    public async Task GetLatest_ReturnsMostRecent()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Msg1");
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Msg2");
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Msg3");
    ...
    public async Task GetPinned_ReturnsOnlyPinned()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg1 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Not pinned");
        var msg2 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Pinned one");
    ...
    public async Task HasOlder_True_WhenOlderExists()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg1 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Older");
        var msg2 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Newer");
    ...
    public async Task HasOlder_False_WhenNoOlder()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg1 = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Only message");
    ...
    public async Task ExistsInChat_True()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Exists");
    ...
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat1 = await DbContextFactory.SeedChatAsync(Context, user.Id, ChatType.Chat, "Chat1", [user.Id]);
        var chat2 = await DbContextFactory.SeedChatAsync(Context, user.Id, ChatType.Chat, "Chat2", [user.Id]);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat1.Id, user.Id, "In chat1");
    ...
    public async Task SoftDelete_MarksDeleted()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Will be deleted");
    ...
    public async Task PinAsync_SetsPinnedAt()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Pin me");
    ...
    public async Task UnpinAsync_ClearsPinnedAt()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Unpin me");
    ...
    public async Task GetForwardedToChatIds_ReturnsDistinctChatIds()
    {
        ...
        var chat1 = await DbContextFactory.SeedChatAsync(Context, user.Id, ChatType.Chat, "Chat1", [user.Id]);
        var chat2 = await DbContextFactory.SeedChatAsync(Context, user.Id, ChatType.Chat, "Chat2", [user.Id]);
        var original = await DbContextFactory.SeedMessageAsync(Context, chat1.Id, user.Id, "Original");
        await DbContextFactory.SeedMessageAsync(Context, chat2.Id, user.Id, "Forwarded", forwardFromId: original.Id);
        await DbContextFactory.SeedMessageAsync(Context, chat2.Id, user.Id, "Also forwarded", forwardFromId: original.Id);
    ...
    public async Task GetChatCounts_ReturnsCorrectCounts()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        var msg = await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "With file");
    ...
    public async Task GetChatCounts_WithCutoff_FiltersByDate()
    {
        var user = await DbContextFactory.SeedUserAsync(Context, "sender", "pass");
        var chat = await DbContextFactory.SeedChatAsync(Context, user.Id);
        await DbContextFactory.SeedMessageAsync(Context, chat.Id, user.Id, "Old message");
