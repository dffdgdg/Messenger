...
public interface IGlobalHubConnection : IAsyncDisposable
{
...
public sealed class GlobalHubConnection(IAuthManager authManager,INotificationService notificationService, ISettingsService settingsService) : IGlobalHubConnection
{
    ...
    public async Task ConnectAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Статус пользователей - теперь просто int
            ...
            // Обновление счётчика непрочитанных - два параметра int
    ...
    public async Task MarkChatAsReadAsync(int chatId)
    {
        ...
        try
        {
            // Сразу обнуляем локально для мгновенного отклика UI
    ...
    private void OnNotificationReceived(NotificationDTO notification)
    {
        ...
        // Если это не текущий чат - увеличиваем счётчик
