...
public partial class MainMenuViewModel : BaseViewModel, IChatNavigator
{
    ...
    public MainMenuViewModel(MainWindowViewModel mainWindowVm, IApiClientService api, IPlatformService platformService, IAuthManager auth, IChatsViewModelFactory chatsFactory,
        IServiceProvider sp, IGlobalHubConnection globalHub, ICallHubConnection callHub, ActiveCallStore activeCallStore, IDrawerService drawerService)
    {
    ...
    private async void OnUserRoleUpdated(UserRole role)
    {
        ...
        if ((currentIndex == 4 || currentIndex == 6) && !IsAdminSectionVisible && !IsDepartmentSectionVisible)
        {
            ...
            await notify.ShowAsync("Права изменены", "Ваши права администратора были отозваны. Вы переведены в основной раздел.",
    ...
    private void OnIncomingCall(CallInviteDto invite) => Dispatcher.UIThread.Post(async () =>
    {
        try
        {
            if (!_callHub.IsConnected)
            {
                ...
                if (!_callHub.IsConnected) return;
            }
    ...
    public async Task ShowEditGroupDialogAsync(ChatDto chat, Action<ChatDto>? onUpdated = null)
    {
        try
        {
            ...
            await _mainWindowVm.ShowDialogAsync(new ChatEditDialogViewModel(_api, _platformService, UserId, chat, members, _auth.Session.IsAdmin)
            {
    ...
    public async Task ShowCreateGroupDialogAsync(Action<ChatDto>? onCreated = null)
    {
        try
        {
            await _mainWindowVm.ShowDialogAsync(new ChatEditDialogViewModel(_api, _platformService, UserId, isSystemAdmin: _auth.Session.IsAdmin)
            {
