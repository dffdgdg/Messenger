...
namespace Core.Features.Chat.ViewModels.Handlers.InfoPanel;
    ...
    : ChatFeatureHandler(context)
{
    ...
    private void UpdateContactStatus(UserStatusDto status)
    {
        ...
        if (!status.IsOnline && status.LastOnline.HasValue)
            ContactUser.LastOnline = status.LastOnline.Value;
    ...
    private void UpdateMemberStatus(UserStatusDto status)
    {
        ...
        if (!status.IsOnline && status.LastOnline.HasValue)
            member.LastOnline = status.LastOnline.Value;
    ...
    private static void InvalidateAvatarCache(string? relativePath)
    {
        ...
        try
        {
            AppConfig.Services.GetRequiredService<AuthenticatedImageLoader>().InvalidateByRelativePath(relativePath);
        }
        catch (Exception ex)
        {
    ...
    internal static string? FormatLastSeen(UserDto contact)
    {
        ...
        var lastOnlineUtc = DateTime.SpecifyKind(contact.LastOnline.Value, DateTimeKind.Utc);
        var elapsed = DateTimeOffset.UtcNow - new DateTimeOffset(lastOnlineUtc);
        ...
        return elapsed.TotalMinutes switch
        {
            ...
            _ => $"был(а) {lastOnlineUtc.ToLocalTime():dd.MM.yyyy}"
        };
    }
