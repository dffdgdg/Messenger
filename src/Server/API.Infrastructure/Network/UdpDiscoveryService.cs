...
public sealed partial class UdpDiscoveryService(ILogger<UdpDiscoveryService> logger, IConfiguration configuration) : BackgroundService
{
    ...
    private const string VirtualBoxSubnetPrefix = "192.168.56.";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                await ProcessDiscoveryRequestAsync(udp, result);
            }
    ...
    private async Task ProcessDiscoveryRequestAsync(UdpClient udp, UdpReceiveResult result)
    {
    ...
    private string BuildResponseString(IPAddress requesterIp)
    {
        ...
        return string.IsNullOrEmpty(responseIp) ? $"{ResponsePrefix}{ApiPort}" : $"{ResponsePrefix}{ApiPort}:{responseIp}";
    }
    ...
    private string? FindSubnetIp(IPAddress requesterIp)
    {
        try
        {
            ...
            return FindIpInSubnet(requesterIp, filterType: null);
        }
        catch (Exception ex)
        {
            LogDiscoveryError(ex);
    ...
    private static string? FindFallbackIp()
    {
        try
        {
            return NetworkInterface.GetAllNetworkInterfaces().Where(IsValidFallbackInterface).OrderByDescending(i => Array.IndexOf(PriorityInterfaceTypes, i.NetworkInterfaceType))
                .SelectMany(i => i.GetIPProperties().UnicastAddresses).Where(IsValidUnicastAddress).Select(a => a.Address.ToString()).FirstOrDefault();
        }
        catch
        {
            return null;
    ...
    private static IEnumerable<NetworkInterface> GetValidInterfaces(NetworkInterfaceType? filterType)
        => NetworkInterface.GetAllNetworkInterfaces().Where(i =>i.OperationalStatus == OperationalStatus.Up
            ...
            && (!filterType.HasValue || i.NetworkInterfaceType == filterType.Value) && !IsVirtualInterface(i.Name));

    private static string? GetMatchingUnicastAddress(NetworkInterface iface, IPAddress requesterIp)
        => iface.GetIPProperties().UnicastAddresses.Where(a => a.Address.AddressFamily == AddressFamily.InterNetwork
            && a.IPv4Mask != null && IsValidUnicastAddress(a)).FirstOrDefault(a => IsInSameSubnet(a.Address, requesterIp, a.IPv4Mask))
            ?.Address.ToString();
    ...
    private static bool IsValidUnicastAddress(UnicastIPAddressInformation addr)
        => addr.Address.AddressFamily == AddressFamily.InterNetwork
            && !addr.Address.ToString().StartsWith(VirtualBoxSubnetPrefix, StringComparison.Ordinal);
