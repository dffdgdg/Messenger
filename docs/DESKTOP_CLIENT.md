...
Для desktop-уведомлений используется **собственный overlay-host внутри MainWindow**, а не нативные системные toast API. Это дает одинаковое поведение на **Windows** и **Linux** и оставляет точку расширения под будущую обработку клика по уведомлению (например, переход в конкретный чат).
...
- `onClick` callback в сервисе пока опционален и предназначен как задел для будущей навигации/аналитики.
